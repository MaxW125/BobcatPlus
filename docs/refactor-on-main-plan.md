...
**Commits landed:** 5 of 9 (`3b9ccef`, `64c817d`, `021e87a`, `a3f6086`, commit 5 below).
**Owner.** Aidan + AI sessions (Opus/API for commits 4–8, Auto for commit 9).
**Gate discipline.** Commits 3–8 require a Chrome browser smoke before the next commit stacks on top. 132 unit tests must stay green on every commit.
...
| 5   | 🟡    | (pending) | Moved `bg/studentInfo.js` (`getStudentInfo`, `getDegreeAuditOverview`, `getAuditData` with RequirementGraph wiring + legacy `findNeeded` fallback, `fetchCourseLinkFromDW` wildcard-expansion helper). Moved `bg/registration.js` (`getCurrentSchedule` with `registrationHistory` fallback for closed terms, `openLoginPopup` at `/saml/login` per D19, synchronizer-token cache, SAML tab resolver, registration-event normalizer). `background.js` drops ~1600 lines; now a thin orchestrator holding only plans + `runAnalysis` + `onMessage` router. 132 tests green, Node SW-module-graph smoke (11 modules: FIFO + cache + export surface) ✓. | **Pending manual smoke: closed-term schedule loads (fall 2024 or earlier) + `auditDiagnostics.parity` spot-check on ≥3 real audits.** |
| 6   | ⬜      | —         | Move `bg/plans.js`.                                                                                                                                                                                                                                                      | Tests green + save/load/delete a Banner plan.                                    |
| 7   | ⬜      | —         | Move `bg/analysis.js` (split if > 400 lines per Deviation A). Every `bail()` verbatim. Add `bail()` contract test against the new module (using `tests/mocks/chrome.js`). Slim `background.js` to thin router.                                                           | Tests green + full smoke: auth → term → eligible → AI → lock/save.               |
...
4. `git show 021e87a` — SW ES-module flip (baseline for commits 4+)

Task for next chat: **Commit 6** — move `bg/plans.js` (Banner plan CRUD + `fetchPlanCalendar`). Keep every POST through `withSessionLock`, preserve synchronizer-token extraction, keep plan-events calendar normalizer intact. End with tests green + save / load / delete a Banner plan smoke.

_(Previous chat opener kept for history — commit 4 was "move `bg/constants.js`, `bg/cache.js`, `bg/session.js`, `bg/bannerApi.js`, `bg/prereqs.js`"; commit 5 was "move `bg/studentInfo.js` + `bg/registration.js`".)_
