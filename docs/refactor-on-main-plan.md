...
**Commits landed:** 6 of 9 (`3b9ccef`, `64c817d`, `021e87a`, `a3f6086`, `f78264a`, `f1738b6`).
...
| 6   | ✅      | `f1738b6` | Moved `bg/plans.js` — Banner Plan Ahead CRUD (`saveManualPlanToTxst`, `getBannerPlanItems`, `getAllBannerPlans`, `fetchPlanCalendar`, `deleteTxstPlan`, `getBannerPlanEvents`) + helpers. `background.js` now ~550 lines (`runAnalysis` + `onMessage`). 132 tests green.                                                                                                                                                                                                                                                                                            | Manual smoke: save/load/delete a Banner plan.                                             |
...
Task for next chat: **Commit 7** — move `bg/analysis.js` (`runAnalysis` with every `bail()` guard verbatim; split if >400 lines per Deviation A). Slim `background.js` to thin router + `analysisGeneration`. Add bail() contract test. Gate: tests green + full smoke (auth → term → eligible → AI → lock/save).
