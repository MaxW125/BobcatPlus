...
| `extension/tab.js`                  | ~227           | ES module tab shell: imports `tab/*`, boot IIFE, term `<select>` handler, `?login=1` toolbar handoff. Loads saved plans + **term-scoped** calendar prefs (`termPrefs.js`) after `getTerms`, hydrates again on term change.                                 |
...
| `termPrefs.js`    | ~138           | `calendarBlocksByTerm` / `avoidDaysByTerm` in `chrome.storage.local`, migrate legacy flat keys, hydrate + persist per Banner term code (SCRUM-21).                                           |
| `ai.js`           | ~733           | Chat, `applyAction`, schedule cards, toolbar, blocks/avoid days (persists via `termPrefs.js`); dynamic `import("./overview.js")` breaks a load-time cycle.                                  |
