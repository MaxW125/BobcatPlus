...
| T2 — Catalog scraper | **S1** | `scripts/catalog/scrape-majors.js` + `degree-combinations.json`. | Independent of T3. |
| T3 — DevTools reverse-engineer DW What-If | **S2** | `docs/plans/whatif-endpoint.md`. | Human DevTools session on Aidan's account. |
| T4 — What-If audit driver | **S3** | `scripts/whatif/pull-audits.js` + run log + gitignored dump. | Gated on T3. |
...
1. **Commit this plan.** (This turn.)
2. **Open Jira epic + child tasks** per §8. (This turn.)
3. **Aidan runs T3 (DevTools session, phase S2)** against his own DW
   account and fills in `docs/plans/whatif-endpoint.md`. No code.
4. **Assistant writes T2 (catalog scraper, phase S1)** — independent of
   T3, can run in parallel.
5. **Only then:** T4 driver, T5 inventory, T6 model update.
