...
function parseSubjectCourseFromRow(section, fallbackSubject, fallbackCourseNumber) {
...
function resolveSectionNumberFromSearchRow(section) {
  ...
  if (m) return m[1];
  if (section.sequenceNumber != null && String(section.sequenceNumber).trim() !== "") {
...
function extractPlanNumberFromBatchResponse(payload) {
  ...
  const pools = [];
  for (const key of ["data", "create", "records", "rows", "entities", "plans"]) {
...
function buildCreatePlanRowFromSection(section, term, planName, subject, courseNumber) {
  ...
  const scheduleTypeDescription =
    section.scheduleTypeDescription || "Lecture";
...
async function runAnalysis(sendUpdate, termCodeOverride) {
  ...
  sendUpdate({ type: "status", message: "Resolving semester for section search..." });
  let term;
  if (termCodeOverride) {
...
function extractPlanHeaders(html) {
  ...
  try {
    ...
    console.warn("[BobcatPlus] extractPlanHeaders: plans array parse failed:", e.message);
    return [];
  }
...
async function getAllBannerPlans(term) {
  try {
    ...
    const selectPlanHtml = await fetchPlanHtml(
      "/ssb/plan/selectPlan?uniqueSessionId=" + encodeURIComponent(uniqueSessionId),
    );
    const planHeaders = extractPlanHeaders(selectPlanHtml);
    console.log("[BobcatPlus] getAllBannerPlans:", planHeaders.length, "plans:", planHeaders.map((p) => p.name));
...
async function fetchPlanCalendar(term, planCourses) {
  ...
  console.log("[BobcatPlus] fetchPlanCalendar: planCourses sample:", JSON.stringify(planCourses.slice(0, 2)));
  ...
  for (const course of planCourses) {
    ...
    if (!courseMap.has(key)) {
      courseMap.set(key, { subject: course.subject, courseNumber: course.courseNumber, crns: new Set() });
    }
    courseMap.get(key).crns.add(crn);
  }
  console.log("[BobcatPlus] fetchPlanCalendar: courseMap:", [...courseMap.entries()].map(([k,v]) => k + " => CRNs:" + [...v.crns].join(",")));
  ...
  for (const { subject, courseNumber, crns } of courseMap.values()) {
    try {
      ...
      await fetch(REG_BASE + "/ssb/classSearch/resetDataForm", { method: "POST", credentials: "include" });
      ...
      const res = await fetch(
        REG_BASE + "/ssb/searchResults/searchResults",
      ...
      const returnedCRNs = data.data.map((s) => String(s.courseReferenceNumber || ""));
      console.log("[BobcatPlus] fetchPlanCalendar:", subject, courseNumber, "- want:", JSON.stringify([...crns]), "- got:", JSON.stringify(returnedCRNs));
  ...
  console.log("[BobcatPlus] fetchPlanCalendar: built", events.length, "events for", courseMap.size, "courses");
  return events;
}
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.action === "getCourseSections") {
    searchCourse(message.subject, message.courseNumber, message.term)
      ...
      .catch((e) =>
        sendResponse({ sections: [], found: false, error: e.message || String(e) }),
      );
    return true;
  }
